...
export default function App() {
  ...
  // Preload background images once per change (avoid creating Image every frame)
  useEffect(() => {
    bgImageRef.current = null;
    if (bgType !== "image" || !bgUrl) return;
  ...
  const handleFileUpload = async (e, type) => {
    ...
    try {
      ...
      if (type === "audio") {
        ...
        // Tear down WebAudio graph. A MediaElementSource can only be created once per <audio> element,
        // so we also force React to recreate the element via key.
